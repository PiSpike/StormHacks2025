...
while run:
    ...
    if keys[pygame.K_RIGHT]:
        if player_x < win.get_width() - width:  # Prevent moving off the right edge
            player_x += vel
...
while show_result:
    ...
    text = myfont.render(str(expr), 1, (0,0,0))
    text_rect = text.get_rect(center=(win.get_width()/2, win.get_height()/2-150))
    win.blit(text,text_rect)
    font = pygame.font.Font("assets/cambria-math.ttf", 34)
    if divided_by_zero:
        ...
        button_font = pygame.font.Font("assets/cambria-math.ttf", 28)
